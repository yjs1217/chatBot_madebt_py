...
### 💡 팁
* 깃허브 README에 적용한 뒤, `timetable.json` 파일이 정상적으로 읽히고 쓰이는지 **실제 코드 상의 파일 경로 설정(`open('timetable.json', ...)` 등)이 상대 경로로 잘 잡혀있는지** 한 번 더 체크해 보시면 완벽합니다! (간혹 절대 경로로 되어 있으면 타인 PC에서 파일 저장 시 에러가 날 수 있습니다.)
